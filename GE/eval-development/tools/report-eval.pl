...
my $evaluator  = '/space/home/genia/public_html/SharedTask/eval/tools/a2-evaluate.pl';
my $name       = 'Anonymous Team';
my $gdir       = './gold';
my $tfile      = '/space/home/genia/public_html/SharedTask/eval/tools/results-team.tmpl';
...
if (!-d $gdir) {print STDERR "Cannot find gold directory: $gdir."; exit}
...
#for task 2
my ($result2, $result2s, $result2sp, $result2dp, $result2dsp) = ('N/A', 'N/A', 'N/A', 'N/A', 'N/A');
...
&PrintResult($name, $tfile,
	     $result1, $result1s, $result1sp, $result1dp, $result1dsp,
	     $result2, $result2s, $result2sp, $result2dp, $result2dsp,
...
sub PrintResult {
    my ($name, $tfile,
	$result1, $result1s, $result1sp, $result1ap, $result1asp,
	$result2, $result2s, $result2sp, $result2ap, $result2asp,
	$result3, $result3s, $result3sp, $result3ap, $result3asp) = @_;
    my $template = HTML::Template->new(filename => $tfile);

    $template->param(RESULT1 => $result1, RESULT1S => $result1s, RESULT1SP => $result1sp, RESULT1AP => $result1ap, RESULT1ASP => $result1asp,
		     RESULT2 => $result2, RESULT2S => $result2s, RESULT2SP => $result2sp, RESULT2AP => $result2ap, RESULT2ASP => $result2asp,
		     RESULT3 => $result3, RESULT3S => $result3s, RESULT3SP => $result3sp, RESULT3AP => $result3ap, RESULT3ASP => $result3asp);
